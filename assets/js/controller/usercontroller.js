angular
	.module('userctrl', [ 'userdata.service' ])
...
function createdRenjaController($scope, RenjaService, $window, helperServices) {
	...
	RenjaService.getKegiatan().then((x) => {
		...
		if (x.data) {
			...
			$scope.sumberanggaran = $scope.datas.sumberanggaran.find(
				(x) => x.iddetailrencanabiaya == $scope.datas.data.iddetailrencanabiaya
			);
	...
	$scope.simpan = () => {
		...
		if (!$scope.model.idPeriodeRenker) $scope.model.idPeriodeRenker = $scope.datas.periode.idPeriodeRenker;
		var fd = new FormData();
		if ($scope.myFile) {
	...
	$scope.showvalidasi = () => {
		if (isEmpty($scope.model)) swal('Information!', 'Tidak ada data yang divalidasi', 'error');
		else {
			$('#waning-validasi').modal('show');
		}
	};
	...
	$scope.addJalan = (item) => {
		$scope.datajalan = {};
		$scope.datajalan.idrt = item.idrt;
		$('#addjalan').modal('show');
	};
	$scope.simpanJalan = () => {
	...
	HomeService.get().then((x) => {
		...
		$scope.datas.forEach((element) => {
			...
			if (element.kegiatan.length > 0) {
				element.kegiatan.forEach((itemkegiatan) => {
					...
					$scope.TotalAnggaranDiterima +=
						itemkegiatan.status == 'Disetujui' ? parseFloat(itemkegiatan.nominal) : 0;
					$scope.TotalAnggaranDiTolak +=
						itemkegiatan.status == 'Batal' ? parseFloat(itemkegiatan.nominal) : 0;
		...
		var config = {
			...
			data: {
				datasets: [
					{
						data: [ $scope.TotalDiterima, $scope.TotalDitolak ],
						backgroundColor: [ window.chartColors.green, window.chartColors.red ],
						label: 'Dataset 1'
					}
				],
				labels: [ 'Usulan Diterima', 'Usulan Ditolak' ]
			},
			options: {
				responsive: true
			}
