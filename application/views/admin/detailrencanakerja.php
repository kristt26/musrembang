      ...
      <div class="card-body">
        <form ng-submit="simpan()">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <select ng-disabled="model.status == 'Final'" class="form-control form-control-sm select2" ng-options="item as item.NamaRencanaBiaya for item in datas.sumberanggaran" style="width:100%;" ng-model="sumberanggaran" id="sumberanggaran" ng-change="model.nominal1 = sumberanggaran.nominal1; model.idRencanaBiaya = sumberanggaran.idRencanaBiaya; model.NamaRencanaBiaya = sumberanggaran.NamaRencanaBiaya; model.iddetailrencanabiaya = sumberanggaran.iddetailrencanabiaya;">
                <option></option>
              </select>
            </div>
          </div>
          <div class="form-group row">
            ...
            <div class="col-sm-10">
              <select ng-disabled="model.status == 'Final'" class="form-control form-control-sm select2" ng-options="item as item.NamaBidangSkpd for item in datas.bidangskpd" style="width:100%;" ng-model="bidangskpd" id="bidangskpd" ng-change="model.idbidangskpd = bidangskpd.idbidangskpd; model.NamaBidangSkpd = bidangskpd.NamaBidangSkpd;">
                <option></option>
              </select>
            </div>
          </div>
        ...
        <!-- <div class="col-md-12"> -->
          <button ng-show="model.status != 'Final' && model.status != 'Batal'" type="button" class="btn btn-block bg-gradient-primary" ng-click="validasi()">Validasi</button>
          <button ng-show="model.status == 'Final' && model.status != 'Batal'" type="button" class="btn btn-block bg-gradient-primary" disabled>Validasi</button>
          <button ng-show="model.status != 'Final' && model.status != 'Batal'" type="button" class="btn btn-block bg-gradient-warning" ng-click="kembalikan()">Kembalikan</button>
          <button ng-show="model.status == 'Final' && model.status != 'Batal'" type="button" class="btn btn-block bg-gradient-warning" disabled>Kembalikan</button>
          <button ng-show="model.status != 'Final' && model.status != 'Batal'" type="button" class="btn btn-block bg-gradient-danger" ng-click="tolak()">Tolak</button>
          <button ng-show="model.status == 'Final' && model.status != 'Batal'" type="button" class="btn btn-block bg-gradient-danger" disabled>Tolak</button>
        <!-- </div> -->
      </div>
    </div>
  </div>
